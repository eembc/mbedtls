...
class CAliasTable:
    ...
    def clone (self, dest_pointer, source_pointer, first_desc):
        if dest_pointer in self.context_to_alias:
            print("Warning: cloning existing context: %s into %s" % (source_pointer, dest_pointer))
        alias = self.base_add(dest_pointer)
        self.description(alias, first_desc)
...
class CParserLibrary:
    ...
    def helper_add_ctx(self, payload, first_desc):
        """ Always add a preliminary description, it may be modified later """
        if payload['dir'] != 'enter':
            return
        ctx = payload['arg0']
        alias = self.trace_processor.aliases.add(ctx)
        self.trace_processor.aliases.description(alias, first_desc)
    ...
    def mbedtls_ssl_handshake_step(self, payload):
        if payload['dir'] != 'enter':
            return
        self.trace_processor.change_state(int(payload['arg0']))

    # AES ECB Functions

    def mbedtls_aes_init(self, payload):
        self.helper_add_ctx(payload, '_aes')
    ...
    def mbedtls_ccm_init(self, payload):
        self.helper_add_ctx(payload, '_ccm')
    ...
    def mbedtls_ecdh_init(self, payload):
        self.helper_add_ctx(payload, "_ecdh")
    ...
    def mbedtls_ecdsa_init(self, payload):
        self.helper_add_ctx(payload, "_ecdsa")
    ...
    def mbedtls_gcm_init(self, payload):
        self.helper_add_ctx(payload, "_gcm")
    ...
    def mbedtls_sha256_init(self, payload):
        self.helper_add_ctx(payload, "_sha")
    ...
    def mbedtls_sha256_clone(self, payload):
        ...
        src = payload['arg0']
        dst = payload['arg1']
        self.trace_processor.aliases.clone(src, dst, "_sha")

    def mbedtls_sha256_update(self, payload):
        if payload['dir'] != 'enter':
            return
        ctx = payload['arg0']
        numbytes = int(payload['arg2'], 16)
...
def main ():
    ...
    print("\nAlias table (leading underscore means no usage was recorded)")
    print("------------------------------------------------------------")
    for i in range (0, trace_processor.aliases.current_alias):
        print("%05d % 15s % 15s" % (
            i,
